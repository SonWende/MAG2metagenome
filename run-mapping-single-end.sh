...
files="single/end/files.txt"

sample=$(sed -n "$SLURM_ARRAY_TASK_ID"p $files)

basedir=
r1=$basedir/reads/${sample}.fastq

# run mapping
bbmap.sh build=1 in=$r1 idfilter=0.97 outm=$basedir/${sample}.mapped.sam statsfile=$basedir/${sample}.statsout threads=80 
# convert to bam
